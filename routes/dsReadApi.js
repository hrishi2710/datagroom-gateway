...
router.post('/getWorkflowTransitionFieldsForJira', async (req, res, next) => {
    let request = req.body
    console.log('getWorkflowTransitionFieldsForJira:', req.body)
    let allowed = await AclCheck.aclCheck(request.dsName, request.dsView, request.dsUser);
    if (!allowed) {
        res.status(415).json({});
        return
    }
    let response = await Jira.getWorkflowTransitionFieldsForJira(request.jiraId)
    if (response && Object.keys(response).length != 0) {
        res.status(200).json(response)
    } else {
        res.status(415).json({})
...
router.post('/view/changeJiraStatus', async (req, res, next) => {
    let request = req.body;
    console.log("In changeJiraStatus: ", request);
    let allowed = await AclCheck.aclCheck(request.dsName, request.dsView, request.dsUser);
    if (!allowed) {
        res.status(415).json({});
        return
